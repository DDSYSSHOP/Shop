...
$vertical_menu = "SELECT `virtuemart_category_id`,title,parent_id,id,`path`,alias FROM `mx_menu` WHERE  parent_id='".$q."'" or die("Ошибка запроса строки " . mysqli_error($link));
...
if (235 == $_REQUEST["q"]) {
  ...
  if ($rows > 0) {
   //  for ($i = 0 ; $i < $rows ; ++$i)
	...
	$vertical_menu1 = "SELECT  `virtuemart_category_id`,`id`,`title`,`alias`,`path`,`link`,`parent_id`, `level`,`menutype` FROM `mx_menu` WHERE `parent_id`='$a'" or die("Ошибка запроса строки " . mysqli_error($link));
