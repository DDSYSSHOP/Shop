...
$vertical_menu = "SELECT `virtuemart_category_id`,title,parent_id,id,alias FROM `mx_menu` WHERE `menutype`='mainmenu' AND parent_id='".$q."'" or die("Ошибка запроса строки " . mysqli_error($link));
...
if (235 == $_REQUEST["q"]) {
  ...
  if ($rows > 0) {
	 ...
	 $row_vertical_menu = mysqli_fetch_row($sql); 
	 //$row_vertical_menu[3]
	...
	$rows1 = mysqli_num_rows($sql1);
     ...
     for ($z = 0 ; $z < $rows1 ; ++$z)
	 	 {
	 $row_vertical_menu1 = mysqli_fetch_row($sql1); 
	 echo "<a href='#$row_vertical_menu1[4]' onclick='openMenu(event,  $row_vertical_menu1[0])'>$row_vertical_menu1[1]</a>";
